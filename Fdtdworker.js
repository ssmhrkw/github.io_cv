// Fdtdworker.js — FDTD本体（SSSS, 合力=1 N 矩形パルス）
self.onmessage = function(ev){
  const msg = ev.data; if(msg.type!=='start') return;
  const cfg = msg.cfg;
  ...
  // 合力=1 N → 面荷重 q = 1/A を2ms印加
  const A = dx*dx;
  const q_per_cell = 1.0 / Math.max(A,1e-18);
  ...
  function lap(a){
    ...
    for(let j=1;j<Ny-1;j++) for(let i=1;i<Nx-1;i++){
      const id=idx(i,j);
      out[id]=(a[id-1]+a[id+1]+a[id-Nx]+a[id+Nx]-4*a[id])/(dx*dx);
    }
    return out;
  }
  ...
  const result={ framesCount:frames.length, Nx, Ny, dt, Fs, dx, Lx, Ly, framesBuf, xs:Array.from({length:Nx},(_,i)=>Lx*i/(Nx-1)), ys:Array.from({length:Ny},(_,j)=>Ly*j/(Ny-1)) };
